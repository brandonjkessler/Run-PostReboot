...
process{
    ...
    try{
        ...
        $Action = ($Action), (New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-Command `"Start-Sleep -Seconds 300; Unregister-ScheduledTask -TaskName '$TaskName' -TaskPath '\$TaskPath\' -Confirm:`$false`"")
        ##-- Create Task
